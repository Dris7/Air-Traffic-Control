...
public class Dijkstra<T> {
	...
	public ArrayList<Airport> printPaths(List<Node<String>> nodes) {
		
		ArrayList<Airport> places=new ArrayList<>();
		nodes.forEach(node -> {
			String path = node.getShortestPath().stream().map(Node::getName).map(Objects::toString)
					.collect(Collectors.joining(" -> "));
			for(Node<String> n:node.getShortestPath()) {
				
				places.add(graph.getAirportByName(n.getName()));
			}
